...
function listOfNames (array){
  const namesList =array.map(({name})=>name);
   return namesList;
}
...
function addArtist(arr, string1, string2, string3, string4, string5) {
  const add = [...arr];
  const beckon= {
    name: string1,
    years: string2,
  ...
  console.log('task 6:', addArtist(artists,'Beckon','1988-2022','Web Design','American','Can do it.'));
...
function artistInfo(arr, string1){
  for (let i=0; i<arr.length; i++){
    if (arr[i].name === string1){
      const artistBio= arr[i].bio;
      return artistBio;
    }
    }
}
